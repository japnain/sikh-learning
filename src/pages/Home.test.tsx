...
test('composes the opening hero around the hukamnama CTA without the old action rail', async () => {
  ...
  expect(within(hero).queryByRole('link', { name: /^Meaning/i })).not.toBeInTheDocument()
  expect(within(hero).queryByRole('button', { name: /^Meaning/i })).not.toBeInTheDocument()
  expect(within(hero).queryByRole('link', { name: /^Return/i })).not.toBeInTheDocument()
  expect(within(hero).queryByRole('button', { name: /^Return/i })).not.toBeInTheDocument()
  expect(within(hero).queryByRole('link', { name: /^Saved$/i })).not.toBeInTheDocument()
...
test('keeps the mobile reveal card low while closing the space beneath it', async () => {
  ...
  try {
    ...
    expect(parseFloat(contentOffset)).toBeGreaterThan(15.75)
    expect(parseFloat(contentOffset)).toBeLessThan(18.75)
  } finally {
    Object.defineProperty(window, 'scrollY', { configurable: true, value: 0 })
